...
func testDo() {
	...
	node1 := &corev1.Node{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				"local-pv-provisioner.cybozu.io/pv-spec-configmap": pvSpecConfigMapName,
			},
		},
	}
	node2 := &corev1.Node{
	...
	cleanupResources := func(ctx context.Context, workingNamespace string) error {
		...
		if _, err := ctrl.CreateOrUpdate(ctx, k8sClient, node1, func() error {
			node1.Annotations["local-pv-provisioner.cybozu.io/pv-spec-configmap"] = pvSpecConfigMapName
			return nil
		}); err != nil {
			return err
		}
		...
		useTestFS(map[string]string{
			...
			ctrl.CreateOrUpdate(ctx, k8sClient, pv, func() error {
				pv.ObjectMeta.Annotations = map[string]string{
					"local-pv-provisioner.cybozu.io/volumeMode":       "Block",
					"local-pv-provisioner.cybozu.io/deviceDir":        "/dev",
					"local-pv-provisioner.cybozu.io/deviceNameFilter": ".*",
				}
				return nil
			})
	...
	DescribeTable(
		"Checking that the device detector operates correctly if pv spec configmap is changed",
		...
		Entry(
			...
			func() {
				...
				_, err = ctrl.CreateOrUpdate(ctx, k8sClient, node1, func() error {
					node1.Annotations["local-pv-provisioner.cybozu.io/pv-spec-configmap"] = pvSpecConfigMapName2
					return nil
				})
				Expect(err).NotTo(HaveOccurred())
			},
	...
	DescribeTable("Checking if PVs are conflicting", func(pvSpec *pvSpec, alreadyCreatedPVsAnnotations []map[string]string, expected bool) {
		...
		for _, annotSrc := range alreadyCreatedPVsAnnotations {
			...
			for k, v := range annotSrc {
				annot["local-pv-provisioner.cybozu.io/"+k] = v
			}
		...
		Entry(
			...
			&pvSpec{volumeMode: "Block", deviceDir: "/dir", deviceNameFilter: ".*"},
			[]map[string]string{
				{"volumeMode": "Block", "deviceDir": "/dir", "deviceNameFilter": ".*"},
				{"volumeMode": "Block", "deviceDir": "/dir", "deviceNameFilter": ".*"},
			},
			false,
		),
		Entry(
			...
			&pvSpec{volumeMode: "Filesystem", fsType: "ext4", deviceDir: "/dir", deviceNameFilter: ".*"},
			[]map[string]string{
				{"volumeMode": "Filesystem", "fsType": "ext4", "deviceDir": "/dir", "deviceNameFilter": ".*"},
				{"volumeMode": "Filesystem", "fsType": "ext4", "deviceDir": "/dir", "deviceNameFilter": ".*"},
			},
			false,
		),
		...
		Entry(
			...
			&pvSpec{volumeMode: "Filesystem", fsType: "ext4", deviceDir: "/dir", deviceNameFilter: ".*"},
			[]map[string]string{
				{"volumeMode": "Filesystem", "fsType": "xfs", "deviceDir": "/dir", "deviceNameFilter": ".*"},
			},
			true,
		),
		...
		Entry(
			...
			&pvSpec{volumeMode: "Block", deviceDir: "/dir", deviceNameFilter: ".*"},
			[]map[string]string{
				{"volumeMode": "Block", "deviceDir": "/dir2", "deviceNameFilter": ".*"},
			},
			true,
		),
		Entry(
			...
			&pvSpec{volumeMode: "Filesystem", fsType: "ext4", deviceDir: "/dir", deviceNameFilter: ".*"},
			[]map[string]string{
				{"volumeMode": "Filesystem", "fsType": "ext4", "deviceDir": "/dir2", "deviceNameFilter": ".*"},
			},
			true,
		),
		...
		Entry(
			...
			&pvSpec{volumeMode: "Filesystem", fsType: "ext4", deviceDir: "/dir", deviceNameFilter: ".*"},
			[]map[string]string{
				{"volumeMode": "Filesystem", "fsType": "ext4", "deviceDir": "/dir", "deviceNameFilter": ".*2"},
			},
			true,
		),
	)
	...
	It("should create PV with specified ownerReference", func() {
		...
		for _, tt := range tests {
			...
			if tt.volumeMode == "Filesystem" {
				...
				Expect(pv.ObjectMeta.Annotations["local-pv-provisioner.cybozu.io/fsType"]).To(Equal(tt.fsType))
			} else {
				Expect(pv.ObjectMeta.Annotations).To(HaveLen(3))
				Expect(pv.ObjectMeta.Annotations["local-pv-provisioner.cybozu.io/fsType"]).To(Equal(""))
			}
			Expect(pv.ObjectMeta.Annotations["local-pv-provisioner.cybozu.io/volumeMode"]).To(Equal(tt.volumeMode))
			Expect(pv.ObjectMeta.Annotations["local-pv-provisioner.cybozu.io/deviceDir"]).To(Equal(deviceDir))
			Expect(pv.ObjectMeta.Annotations["local-pv-provisioner.cybozu.io/deviceNameFilter"]).To(Equal(deviceNameFilter))

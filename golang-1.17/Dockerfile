...
RUN rm -rf /usr/local/go \
    && curl -sfL https://dl.google.com/go/go${GO_VERSION}.linux-amd64.tar.gz \
    ...
    && GOBIN=/usr/local/bin go install github.com/gostaticanalysis/nilerr/cmd/nilerr@latest \
    && rm -rf /go \
    && mkdir -p /go/src

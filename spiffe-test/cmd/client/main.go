...
func (o *option) parse() error {
	...
	var err error
	o.parsedSrvID, err = spiffeid.FromString(o.serverID)
	if err != nil {
...
func newRootCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&opt.serverID, "server-id", "spiffe://example.com/server", "Expected server SPIFFE ID")
...
func runClient(ctx context.Context, opt *option) error {
	...
	slog.Info("Client created", "transport", string(opt.transportMode), "authMode", string(opt.authMode))
...
func createClient(opt *option, workloadClient *spiffe.WorkloadClient) (transport.HelloClient, error) {
	...
	tlsConfig := clientTLSConfig(opt.authMode, tlsProvider, opt.parsedSrvID)
...
func runRequestLoop(ctx context.Context, opt *option, client transport.HelloClient, workloadClient *spiffe.WorkloadClient) error {
	...
	for {
		...
		slog.Info("Making request", "count", count, "server", opt.serverAddr, "transport", string(opt.transportMode))
